...
class WelcomeFrame(ctk.CTkFrame):
      def __init__(self,master):
            ...
            button(master=self,string="continue",command= lambda : self.master.render_frame(GamePlayFrame,level = self.current_level,is_existing = True)).pack(pady=(20,0))
            #Play a new game
            button(master=self,string="new game",command = lambda : self.master.render_frame(GamePlayFrame,level = self.current_level,is_existing = False)).pack(pady=(20,0))
      ...
      def switch_level(self,distance):
            ...
            if Config.saved_game_exists(self.current_level):
                  pass
      
      def arrow(self,text,command):
            ctk.CTkButton(
                  self.level_select_frame,
                  text=text,
                  width=10,
                  height=10,
                  command=command,
...
def button(master,string,command):
